...
class DataGenCustom:
    ...
    def __init__(self, path, split_ratio, x, y, color_space='rgb'):
        ...
        self.path_train_labels = path
        self.path_test_images = path
        self.path_test_labels = path
...
def save_results_custom(np_array, color_space, outpath, test_label_filenames_list, base_url):
    ...
    new_file_name = file_without_ext + "_prediction_" + timestr + "." + extension
    # predict_img = np.reshape(predict_img,(predict_img[0],predict_img[1]))
    pred = np_array[0]
    cv2.imwrite(outpath + new_file_name, pred * 255.)

    img = cv2.imread(outpath + new_file_name)
    #cv2.imshow('Image', img)
    ...
    value = {
        ...
        "white_pixels": float(number_of_white_pix),
